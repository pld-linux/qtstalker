...
cat Makefile | sed -e "s:.(QTDIR)/lib:%{_prefix}/X11R6/lib/qt:g" |\
	sed -e "s:.(QTDIR)/include:%{_prefix}/X11R6/include/qt:g"|\
	sed -e "s:.(QTDIR)/bin/moc:%{_prefix}/X11R6/bin/moc:g"> Makefile.pld
...
do
	if [ "$line" == "make" ]; then
		echo "cat Makefile |\
		  sed -e 's:.(QTDIR)/lib:%{_prefix}/X11R6/lib/qt:g' |\
		  sed -e 's:.(QTDIR)/include:%{_prefix}/X11R6/include/qt:g'|\
		  sed -e 's:.(QTDIR)/bin/moc:%{_prefix}/X11R6/bin/moc:g'> Makefile.pld" >> compile.pld
...
do
        if [ "$line" == "make" ]; then
echo "cat Makefile | sed -e 's:.(QTDIR)/lib:%{_prefix}/X11R6/lib/qt:g' |\
   sed -e 's:.(QTDIR)/include:%{_prefix}/X11R6/include/qt:g'|\
   sed -e 's:.(QTDIR)/bin/moc:%{_prefix}/X11R6/bin/moc:g'> Makefile.pld" >> compile.pld
		echo "mv Makefile.pld Makefile" >> compile.pld
                echo "$line"|sed -e "s/__p_l_d__/ /g" >> compile.pld
...
install -d $RPM_BUILD_ROOT{%{_bindir},%{_libdir}/%{name}/plugins/{indicator,quote}} \
	$RPM_BUILD_ROOT%{_kdedatadir}/pixmaps/hicolor/16x16/apps \
	$RPM_BUILD_ROOT{%{_applnkdir}/Office/Misc,%{_pixmapsdir},/tmp/HTML/HTML}
